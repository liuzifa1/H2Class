...
│  (Pages)   │  │  console   │  │ (SwiftUI)  │  │ (Compat. API) │
└─────┬──────┘  └─────┬──────┘  └─────┬──────┘  └───────┬───────┘
      └───────────────┴───────┬───────┴─────────────────┘
...
- `price`: **versioned** — new price = new immutable row with an effective date and integer-fen amount. Current price is the latest effective date not later than now. Never update or delete a price row; every sale references the exact version it was sold at, so historical income never silently changes.
...
| Runs out | Credits reach 0 | `valid_to` passes |
...
Database triggers reject credit-ledger updates and deletes. For non-trusteeship lessons, attendance deductions are inserted from the current deduction-policy row and the oldest matching positive-balance package (FIFO) inside the same atomic batch as attendance and guardian drafts; missing entitlement aborts the whole batch. Makeup lessons never create an attendance deduction. Trusteeship attendance validates an active date-covering subscription and creates no credit entry.

### 3.5 Scheduling

- `lesson`: class type, teacher, UTC start/end instants, scheduled/completed/cancelled status, optional term, and optional makeup linkage.
- `enrollment`: student ↔ lesson (a 1v4 group lesson = 1 lesson row, 4 enrollments).
- Conflict detection on create/update: teacher double-booked, student double-booked; class capacity is enforced when enrollment is written.
- Teacher weekly availability uses Asia/Shanghai-local weekday and minute-from-midnight slots, with dated exceptions. Closure dates and term bounds are local `YYYY-MM-DD` values; recurring generation skips closures and stays within its term.
- Book, move, and cancel compose the lesson mutation and concrete teacher/guardian message drafts into one atomic D1 batch. Booking does not consume credits.
...
- Wire statuses are `present`, `absent`, `excused_leave`, and `late_cancel`; the seeded policy deducts all except `excused_leave`.
...
- When a student has authorized pickup people, check-out records an exact registered name. An exception uses a separately audited owner-only override; it is never a normal-checkout flag.

### 3.7 Payments & income

- `payment` table is the **only** source of income truth. A scheduled lesson is not income; a payment is.
- Sales and refunds are immutable payment rows tied to the exact `price_id`. Core derives package list price as the sold unit price times purchased credits and records the explicit discount; zero-paid purchases are rejected because the v1 refund model requires a negative money row.
- Package refund value is `floor(original paid amount × remaining credits ÷ purchased credits)`. A readonly quote supplies optimistic values; confirmation rechecks them and atomically appends negative payment and ledger rows. Trusteeship-subscription refunds are intentionally unsupported in v1.
...
Lesson feedback is unique per lesson/student and is persisted atomically with
raw linked-guardian drafts. Medical notes are stored separately from ordinary
person notes and are excluded from list and agent responses; only staff/admin
person detail responses may include them.
...
- Guardians receive owner-provisioned accounts for the read-only parent portal; there is no public self-signup. Students normally do not receive accounts.
