...
apps/agent  "agent" — Cloudflare Worker: OpenAI-compatible agent loop. ZERO business logic; calls core's REST API.
...
Stack versions (pinned in package.json — don't bump without approval): TypeScript 5.x strict, Hono 4, Drizzle 0.45 + D1, Zod **4** (`import { z } from "zod"`), wrangler 4, pnpm workspaces. The agent calls a vendor-neutral OpenAI-compatible Chat Completions endpoint with `fetch`.
...
- Secrets only via wrangler secrets / `.dev.vars` (gitignored). Never in code, config, or docs. `OPENAI_API_KEY` and `CORE_API_TOKEN` exist only on the agent worker.
