...
`apps/agent` is a separate small Node/TypeScript service:
...
- **Hard rule: core data only via the API.** The agent keeps its *own* state (conversations, run history) in its own tables (same Postgres instance, separate schema) and never touches core tables directly.
...
- **Chat plumbing**: the admin console opens the chat against the agent service (`POST /chat`, streamed). The agent validates the owner's session by calling core's `/me` with the forwarded token — only authenticated admins can drive it.

```
apps/agent/src/
├── server.ts     # chat endpoint (streamed), health
├── loop.ts       # agent loop: Claude ↔ tools
...
| Setting | Value | Why |
|---|---|---|
| SDK | `@anthropic-ai/sdk` (TypeScript) | Same language as everything else |
| Model | `claude-opus-4-8` | Multi-constraint scheduling + multi-step workflows |
| Thinking | `thinking: {type: "adaptive"}` | Thinks hard on tricky rearrangements, answers fast otherwise |
| Effort | `output_config: {effort: "high"}` | Correct operations matter more than tokens |
| Streaming | Yes, through to the console | You watch it work |
| Prompt caching | `cache_control` on system prompt; stable tool order | The tool registry is large — caching keeps it cheap |
...
for (const block of response.content) {
  ...
  const result = await tool.execute(block.input); // → api.ts → core REST
...
- Data sent to Anthropic = conversation + tool returns (scoped responses, not bulk dumps); commercial API terms, not used for training.

## 10. Cost

The agent handles most operations now; still small. With caching, a typical workflow conversation ≈ tens of thousands of input tokens (mostly cache reads) + a few K output ≈ **cents per workflow**; a busy month lands in the tens of dollars. Watch it in the daily digest.
