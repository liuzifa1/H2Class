...
4. **课后反馈** — lesson feedback entries once that feature lands (v1: hide the tab if the endpoint 404s).

## Rules

- **Scoping is server-side**: this app only ever calls guardian-scoped endpoints (`/my/...`), which core filters to the authenticated guardian's linked students. If you ever need to pass a `student_id` that isn't from `/my/children`, something is wrong — stop.
