...
- API access: two bases from env — `VITE_CORE_API_URL`, `VITE_AGENT_API_URL`. Bearer token from better-auth login, held in memory + localStorage; attach via a single `fetchJson` helper typed from `@h2class/shared` (never hand-type response shapes).
- All times displayed in Asia/Shanghai; all money displayed from integer cents.
...
2. **Chat** (primary) — conversation list + thread with the agent:
   - Stream from `POST {AGENT}/chat` (SSE): render text deltas live; show tool activity as compact status lines ("查询王老师的空闲时间…"), not raw JSON.
